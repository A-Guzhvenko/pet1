...
def huber_loss(y_true: np.ndarray, y_pred: np.ndarray, delta: float = 1.345):
    """Huber loss function"""
    assert len(y_true) == len(y_pred), 'Data sizes do not match'
    huber_sum = 0
...
def get_metrics_regression(y_test: np.ndarray,
                           ...
                           name: str = None,
                           delta: float = 1.345):
    """Generate a table with metrics for a regression task"""
...
def get_metrics_classification(y_test, y_pred, y_score, name):
    """Generate a table with metrics for a classification task"""
    df_metrics = pd.DataFrame()
